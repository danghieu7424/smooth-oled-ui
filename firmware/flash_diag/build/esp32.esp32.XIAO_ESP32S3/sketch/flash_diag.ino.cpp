...
#line 20 "D:\\all_projects\\rust\\rust\\display_oled\\firmware\\flash_diag\\flash_diag.ino"
void printHex(const uint8_t* data, int len);
#line 31 "D:\\all_projects\\rust\\rust\\display_oled\\firmware\\flash_diag\\flash_diag.ino"
bool testFlashAddress(uint32_t addr, const char* label);
#line 72 "D:\\all_projects\\rust\\rust\\display_oled\\firmware\\flash_diag\\flash_diag.ino"
void setup();
#line 207 "D:\\all_projects\\rust\\rust\\display_oled\\firmware\\flash_diag\\flash_diag.ino"
void loop();
#line 20 "D:\\all_projects\\rust\\rust\\display_oled\\firmware\\flash_diag\\flash_diag.ino"
void printHex(const uint8_t* data, int len) {
    for (int i = 0; i < len; i++) {
...
void setup() {
    ...
    // Đọc Status Register 1 (SR1) bằng API tương thích Arduino Core 2.0.11
    uint32_t sr1 = 0;
    esp_err_t err = spi_flash_read_status(&sr1);
    if (err == ESP_OK) {
